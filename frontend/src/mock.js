...
export const profile = {
  ...
  resumeUrl: "/assets/TCA-Resume-DevOps.pdf",
...
export const projects = [
  {
    ...
    stack: [
      ...
      "Kyverno", "Trivy", "Velero", "Cloudflare", "Tailscale",
    ],
  ...
  },
  {
    id: "project-iris",
    name: "Project Iris",
    subtitle: "Private AI Workspace for Knowledge Operators",
    category: "AI Infrastructure",
    description:
      "A local-first assistant platform for founders, operators, and technical teams that need AI workflows with memory control, tool execution, managed browser actions, TLS and auth hardening, backups, and explicit user governance.",
    stack: ["FastAPI", "React", "Vite", "TLS", "Auth", "Backups", "Memory Governance"],
  ...
  },
  {
    ...
    description:
      "A delivery platform for an AI education business, connecting the TypeScript application, CI path filters, image promotion, GitOps release controls, runtime drift checks, and operational runbooks into one deployable product system.",
    stack: ["TypeScript", "CI/CD", "GitHub Actions", "Kubernetes", "GitOps", "Runtime Images"],
    pillars: ["Delivery", "Automation", "Product Ops"],
    outcomes: ["Cleaner deployable paths", "Image promotion discipline", "Runtime drift checks"],
    signal: "Active business platform",
  ...
  {
    id: "vault-ops",
    name: "Vault Ops Toolkit",
    subtitle: "Secrets Operations for Platform Teams",
    category: "Security Automation",
    description:
      "An operations toolkit for teams that need auditable secret handling across environments, Vault-backed credential workflows, Kubernetes External Secrets integration, bootstrap automation, and practical recovery procedures.",
    stack: ["Python", "Vault", "External Secrets", "Kubernetes", "Shell", "Runbooks"],
    pillars: ["Secrets", "Automation", "Bootstrap"],
    outcomes: ["Secret recovery workflows", "ExternalSecret validation", "Operator-safe automation"],
    signal: "Security-critical tooling",
    repo: "vault-ops",
  ...
  },
  {
    id: "sentinel-copilot",
    name: "ForgeWatch",
    subtitle: "Evidence-First AIOps Copilot for InfraForge",
    category: "AIOps / Platform Operations",
    description:
      "ForgeWatch, implemented in the sentinel-copilot repo, is an internal operational intelligence service for InfraForge. It gathers evidence from Kubernetes, Prometheus, Loki, and Argo CD, then turns alerts and operator questions into structured triage with approval-gated remediation.",
    stack: ["FastAPI", "Ollama", "Kubernetes", "Prometheus", "Loki", "Argo CD", "ChromaDB", "Postgres", "Discord", "ntfy"],
    pillars: ["AIOps", "Evidence", "Approval Gates"],
    outcomes: ["Read-only autonomous investigation", "Human-approved remediation", "Outcome-backed memory"],
    signal: "Internal company control plane",
    repo: "sentinel-copilot",
    repoUrl: "https://github.com/temitayocharles/sentinel-copilot",
    visibility: "Public repo",
    accent: "teal",
    caseStudy: {
      title: "Evidence-first operations for lean platform teams",
      context:
        "ForgeWatch is the operational intelligence layer for InfraForge. It uses tool calling to collect real platform evidence first, then produces triage, digests, watchdog findings, and approval-gated write proposals for operators.",
      decisions: [
        "Keep Kubernetes, Prometheus, Loki, and Argo CD queries autonomous but read-only by default.",
        "Route all mutating actions through propose_write, policy evaluation, and explicit operator approval.",
        "Store outcomes and incident memory so future triage improves without turning the system into an unsafe autonomous executor.",
      ],
      result:
        "ForgeWatch gives InfraForge a company-grade AIOps control plane where AI speeds investigation, but operators retain authority over production state.",
    },
  },
  {
    id: "openleaf",
    name: "OpenLeaf Reader Platform",
    subtitle: "Reference SaaS Architecture for Digital Products",
    category: "Cloud-Native Architecture",
    description:
      "A reference SaaS platform for digital content businesses, modeling API gateway governance, event-driven service boundaries, payments integration, release automation, observability, and reliability testing.",
    stack: ["Kong", "decK", "Python", "React", "RabbitMQ", "MongoDB", "Stripe", "ArgoCD", "Prometheus", "Chaos Mesh"],
    pillars: ["Microservices", "API Gateway", "Chaos"],
    outcomes: ["Gateway governance", "Evented service boundaries", "Reliability testing"],
    signal: "Public architecture demo",
    repo: "OpenLeaf-Reader-Platform",
...
export const projectArchitectures = [
  {
    ...
    title: "TCA InfraForge operating platform",
    repo: "homelab-gitops + infraforge-tofu + vault-ops",
    summary:
      "The company platform that packages Kubernetes delivery, GitOps, secrets, private networking, observability, registry recovery, and disaster recovery into a repeatable operating system for startups and growing teams.",
    businessValue:
    ...
    nodes: [
      ...
      { id: "envoy", label: "Envoy Gateway", layer: "Ingress", x: 27, y: 28, tone: "teal", details: "Gateway API routing for internal products, Harbor, portfolio API, and platform services." },
      { id: "argocd", label: "Argo CD", layer: "Control", x: 27, y: 72, tone: "teal", details: "GitOps reconciler that applies platform applications, sync waves, and ownership boundaries." },
      { id: "vault", label: "Vault", layer: "Security", x: 48, y: 18, tone: "amber", details: "Secrets authority for platform and workload credentials." },
      { id: "eso", label: "External Secrets", layer: "Security", x: 48, y: 45, tone: "teal", details: "Kubernetes bridge that syncs approved secret values into application namespaces." },
      { id: "workloads", label: "Workloads", layer: "Runtime", x: 66, y: 38, tone: "teal", details: "Portfolio API, AI Builders, ForgeWatch, platform services, and client-ready workloads." },
      { id: "harbor", label: "Harbor", layer: "Supply Chain", x: 66, y: 70, tone: "amber", details: "Private container registry with retained Longhorn storage and recovery runbooks." },
      { id: "longhorn", label: "Longhorn", layer: "State", x: 86, y: 72, tone: "amber", details: "Persistent volume layer for registry, databases, and other stateful services." },
      { id: "observability", label: "Observability", layer: "Signals", x: 88, y: 30, tone: "teal", details: "Prometheus, Grafana, Loki, Alertmanager, Netdata, and runbooks for operational visibility." },
    ],
    edges: [
      ["cloudflare", "envoy", "public route"],
      ["tailscale", "argocd", "operator access"],
      ["argocd", "workloads", "sync"],
      ["vault", "eso", "secret source"],
      ["eso", "workloads", "runtime secrets"],
      ["harbor", "workloads", "images"],
      ["workloads", "observability", "metrics logs"],
      ["harbor", "longhorn", "registry state"],
      ["workloads", "longhorn", "persistent state"],
    ],
    flows: [
      "Git commit updates the platform repository, then Argo CD reconciles the desired state into the cluster.",
      "Cloudflare sends public traffic to Envoy Gateway, then Gateway API routes requests to the correct namespace service.",
      "Vault remains the source of truth for credentials while External Secrets delivers only approved runtime values.",
      "Longhorn retained volumes and runbooks make stateful recovery practical after namespace or pod failure.",
    ],
    controls: ["GitOps ownership boundaries", "Vault-backed secrets", "NetworkPolicy segmentation", "Harbor recovery runbook", "Observable route health"],
  },
  {
    id: "project-iris",
    projectId: "project-iris",
    label: "Private AI workspace",
    title: "Project Iris governed AI workspace",
    repo: "project-iris",
    summary:
      "A local-first AI workspace for founders and operators that treats memory, browser actions, tool use, TLS, backup, and identity as governed infrastructure.",
    businessValue:
    ...
    nodes: [
      ...
      { id: "api", label: "FastAPI", layer: "Application", x: 43, y: 48, tone: "teal", details: "Backend boundary for auth, memory operations, tool routing, and workflow state." },
      { id: "auth", label: "Auth + TLS", layer: "Security", x: 24, y: 72, tone: "amber", details: "Identity and transport controls so the workspace is not an open local demo." },
      { id: "memory", label: "Memory Store", layer: "Knowledge", x: 62, y: 25, tone: "amber", details: "Governed long-term memory with explicit user control and backup awareness." },
      { id: "tools", label: "Tool Router", layer: "Execution", x: 63, y: 58, tone: "teal", details: "Explicit tool execution boundary for browser actions, local actions, and workflow helpers." },
      { id: "browser", label: "Managed Browser", layer: "Automation", x: 82, y: 44, tone: "teal", details: "Controlled browser action path for interactive tasks that should remain visible and governable." },
      { id: "backup", label: "Backups", layer: "Recovery", x: 82, y: 75, tone: "amber", details: "Backup and restore path for workspace state, memory, and configuration." },
    ],
    edges: [
      ["user", "web", "session"],
      ["web", "api", "requests"],
      ...
      ["api", "tools", "authorized call"],
      ["tools", "browser", "managed action"],
      ["memory", "backup", "snapshot"],
      ["api", "backup", "config state"],
    ],
    flows: [
      "The user interacts through a private UI, not an uncontrolled public assistant surface.",
      "The API checks auth and routes memory or tool requests through explicit backend boundaries.",
      "Tool execution remains visible and separated from memory so users understand what the assistant can do.",
      "Backups protect workspace state so the system can be recovered after local or runtime failure.",
    ],
  ...
  },
  {
    ...
    title: "AI Builders Academy delivery system",
    repo: "ai-builders-academy + homelab-gitops",
    summary:
      "A business delivery platform for AI education, with app changes, image promotion, GitOps, runtime checks, and operational runbooks tied into one product release lane.",
    businessValue:
      "Shows how an education business can move quickly while keeping deployable boundaries, runtime state, and platform ownership visible.",
    metric: "Deployable paths, image promotion, drift checks",
    tabs: ["Topology", "Flows", "Controls"],
    nodes: [
      { id: "content", label: "Program Content", layer: "Business", x: 8, y: 28, tone: "amber", details: "Curriculum, program assets, and delivery material for AI education products." },
      { id: "frontend", label: "Frontend", layer: "Experience", x: 25, y: 24, tone: "teal", details: "Student and buyer-facing application interface." },
      { id: "api", label: "Backend API", layer: "Application", x: 45, y: 38, tone: "teal", details: "Business logic, enrollment workflows, admin operations, and product APIs." },
      { id: "ci", label: "CI Path Filters", layer: "Delivery", x: 25, y: 73, tone: "amber", details: "Refined deployable path filters prevent unrelated changes from triggering unsafe releases." },
      { id: "registry", label: "Image Registry", layer: "Supply Chain", x: 47, y: 73, tone: "amber", details: "Runtime images are promoted deliberately instead of relying on ambiguous state." },
      { id: "gitops", label: "GitOps Release", layer: "Platform", x: 66, y: 54, tone: "teal", details: "Argo CD applies the promoted release into the target environment." },
      { id: "checks", label: "Runtime Checks", layer: "Quality", x: 84, y: 35, tone: "teal", details: "Drift checks and smoke validation keep deployed behavior visible." },
      { id: "runbooks", label: "Runbooks", layer: "Operations", x: 84, y: 74, tone: "amber", details: "Operational notes describe recovery, deployment, and verification procedures." },
    ],
    edges: [
      ["content", "frontend", "publish"],
      ["frontend", "api", "app calls"],
      ["ci", "registry", "build promote"],
      ["registry", "gitops", "image tag"],
      ["gitops", "api", "rollout"],
      ["gitops", "frontend", "release"],
      ["checks", "runbooks", "evidence"],
      ["api", "checks", "verify"],
    ],
    flows: [
      "Relevant code paths trigger the correct build lane and avoid noisy unrelated deployment activity.",
      "Images are built, promoted, and reconciled through GitOps so release state is reviewable.",
      "Runtime drift checks verify that the deployed product matches the expected operating state.",
      "Runbooks document recovery and validation so the business can keep operating after changes.",
    ],
    controls: ["Deployable path filters", "Image promotion discipline", "Runtime drift checks", "GitOps rollout", "Business runbooks"],
  },
  {
    id: "vault-ops",
    projectId: "vault-ops",
    label: "Secrets operations",
    title: "Vault Ops secrets control plane",
    repo: "vault-ops",
    summary:
      "A secrets operations toolkit for Vault-backed platform teams that need bootstrap, rotation, External Secrets sync, validation, and recovery procedures.",
    businessValue:
      "Turns secrets management into a runbook-backed platform capability that teams can operate safely under pressure.",
    metric: "Vault authority with Kubernetes delivery",
    tabs: ["Topology", "Flows", "Controls"],
    nodes: [
      { id: "operator", label: "Platform Operator", layer: "Human", x: 8, y: 46, tone: "amber", details: "Authorized operator who bootstraps, validates, and rotates platform secrets." },
      { id: "vault", label: "Vault", layer: "Authority", x: 29, y: 30, tone: "amber", details: "System of record for sensitive credentials and environment values." },
      { id: "policy", label: "Policies", layer: "Security", x: 29, y: 70, tone: "teal", details: "Access policy and path structure that defines who can read or write secret material." },
      { id: "eso", label: "External Secrets", layer: "Sync", x: 52, y: 48, tone: "teal", details: "Controller that syncs selected Vault values into Kubernetes secrets." },
      { id: "k8ssecret", label: "K8s Secrets", layer: "Runtime", x: 72, y: 30, tone: "amber", details: "Namespace-local runtime secret objects consumed by workloads." },
      { id: "workloads", label: "Workloads", layer: "Applications", x: 88, y: 52, tone: "teal", details: "Applications that consume secrets without direct Vault coupling." },
      { id: "audit", label: "Validation", layer: "Assurance", x: 72, y: 76, tone: "teal", details: "Shape checks, sync checks, and recovery evidence before rollout dependency." },
    ],
    edges: [
      ["operator", "vault", "bootstrap"],
      ["operator", "policy", "manage"],
      ["policy", "vault", "authorize"],
      ["vault", "eso", "read"],
      ["eso", "k8ssecret", "sync"],
      ["k8ssecret", "workloads", "mount env"],
      ["audit", "eso", "verify"],
      ["audit", "workloads", "release gate"],
    ],
    flows: [
      "Operators place credentials in Vault under known paths and policy boundaries.",
      "External Secrets syncs approved values to Kubernetes namespaces without exposing Vault broadly.",
      "Workloads consume Kubernetes secrets while audit checks confirm the secret shape before rollout.",
      "Recovery procedures restore missing or broken secrets from the authority source instead of guessing values.",
    ],
    controls: ["Vault as source of truth", "ExternalSecret sync validation", "Policy-scoped access", "Recovery runbooks", "No secret guessing"],
  },
  {
    id: "forgewatch",
    projectId: "sentinel-copilot",
    label: "AIOps copilot",
    ...
    nodes: [
      ...
      { id: "ollama", label: "Ollama Loop", layer: "Reasoning", x: 45, y: 25, tone: "teal", details: "Six-round tool-calling loop that decides which evidence sources to query before answering." },
      { id: "executor", label: "Tool Executor", layer: "Execution", x: 45, y: 67, tone: "amber", details: "Runs read-only tools immediately and gates write proposals through policy and approval." },
      { id: "evidence", label: "Evidence Sources", layer: "Telemetry", x: 68, y: 28, tone: "teal", details: "Kubernetes, Prometheus, Loki, and Argo CD clients provide real-time platform evidence." },
      { id: "memory", label: "RAG Memory", layer: "Learning", x: 68, y: 67, tone: "teal", details: "ChromaDB, outcomes, history, and runbooks improve future triage with confirmed context." },
      { id: "policy", label: "Policy Engine", layer: "Safety", x: 84, y: 46, tone: "amber", details: "Classifies write intent, blocks dangerous commands, and forces approval for mutating actions." },
      { id: "notify", label: "Discord / ntfy", layer: "Notification", x: 91, y: 78, tone: "teal", details: "Optional proactive notifications for digest, watchdog, and startup status." },
    ],
    edges: [
      ["operator", "api", "ask alert"],
      ["api", "ollama", "prompt"],
      ["ollama", "executor", "tool call"],
      ["executor", "evidence", "read-only query"],
      ["executor", "policy", "write proposal"],
      ["policy", "operator", "approve deny"],
      ["api", "memory", "outcome"],
      ["memory", "ollama", "context"],
      ["api", "notify", "digest"],
    ],
    flows: [
      "An operator or alert calls the API with a platform question or incident payload.",
      "Ollama selects tools and the executor queries Kubernetes, Prometheus, Loki, or Argo CD for evidence.",
      "If remediation is needed, ForgeWatch creates a write proposal instead of executing it automatically.",
      "Policy evaluates the command, then the operator approves or denies through the writes endpoint.",
      "Outcomes and incident notes feed memory so future responses improve with confirmed operational context.",
    ],
    controls: ["Read-only kubectl allowlist", "No secret reads", "Operator auth", "Policy-gated writes", "Outcome-backed RAG memory"],
  },
  {
    id: "openleaf",
    projectId: "openleaf",
    label: "SaaS reference",
    title: "OpenLeaf digital product platform",
    repo: "OpenLeaf-Reader-Platform",
    summary:
      "A reference SaaS architecture for digital content products with API gateway governance, service boundaries, async messaging, payments, observability, GitOps, and reliability testing.",
    businessValue:
      "Shows how a digital product company can evolve from app code into service ownership, gateway control, and reliability practice.",
    metric: "Gateway, services, events, payments, chaos",
    tabs: ["Topology", "Flows", "Controls"],
    nodes: [
      { id: "reader", label: "Reader UI", layer: "Experience", x: 7, y: 45, tone: "teal", details: "Customer-facing reading and content experience." },
      { id: "gateway", label: "Kong Gateway", layer: "API", x: 25, y: 45, tone: "amber", details: "API gateway governance with route, policy, and service exposure control." },
      { id: "users", label: "User Service", layer: "Service", x: 46, y: 20, tone: "teal", details: "Identity and user profile boundary." },
      { id: "catalog", label: "Book Service", layer: "Service", x: 46, y: 45, tone: "teal", details: "Content catalog and reader domain boundary." },
      { id: "billing", label: "Payments", layer: "Revenue", x: 46, y: 72, tone: "amber", details: "Stripe or payment provider integration for subscriptions and transactions." },
      { id: "events", label: "RabbitMQ", layer: "Events", x: 67, y: 45, tone: "amber", details: "Asynchronous coordination for product events and service communication." },
      { id: "mongo", label: "MongoDB", layer: "Data", x: 86, y: 27, tone: "teal", details: "Document data store for reader and product state." },
      { id: "observe", label: "Prometheus + Chaos", layer: "Reliability", x: 86, y: 70, tone: "teal", details: "Metrics and reliability testing make service behavior visible under stress." },
    ],
    edges: [
      ["reader", "gateway", "https"],
      ["gateway", "users", "identity"],
      ["gateway", "catalog", "content"],
      ["gateway", "billing", "checkout"],
      ["users", "events", "user event"],
      ["catalog", "events", "content event"],
      ["catalog", "mongo", "documents"],
      ["billing", "events", "payment event"],
      ["events", "observe", "signals"],
    ],
    flows: [
      "Readers use the frontend, while Kong keeps service APIs governed through one controlled entry point.",
      "User, catalog, and billing services own separate business boundaries rather than one tangled app surface.",
      "RabbitMQ carries async events so product actions can be processed without coupling every service directly.",
      "Prometheus and chaos testing expose reliability behavior before customers experience hidden failure modes.",
    ],
    controls: ["Gateway governance", "Service boundaries", "Event-driven coordination", "Payment isolation", "Reliability testing"],
  },
];
