...
export const projects = [
  {
    id: "infraforge",
    name: "TCA InfraForge",
    subtitle: "Production-Style Kubernetes Homelab Platform",
    category: "Platform Engineering",
    description:
      "A bare-metal Ubuntu / K3s high-availability platform that behaves like a small production cloud: GitOps delivery, Gateway API ingress, Vault-backed secrets, Longhorn storage, observability, policy controls, backups, and recovery runbooks.",
    stack: [
    ...
    outcomes: ["Synced / Healthy GitOps apps", "Recovered Harbor registry", "Validated domain cutover"],
    signal: "Production-style platform operations",
    repo: "homelab-gitops",
  ...
  },
  {
    ...
    description:
      "A local-first assistant platform with FastAPI and React / Vite, memory governance, tool execution, managed browser capability, TLS / auth hardening, backup validation, and user-facing workflow controls.",
    stack: ["FastAPI", "React", "Vite", "TLS", "Auth", "Backups", "Memory Governance"],
    pillars: ["AI", "Privacy", "Local-first"],
    outcomes: ["Local control model", "Workflow guardrails", "Operational recovery path"],
    signal: "Most recent active build",
    repo: "project-iris",
    visibility: "Private case study",
    accent: "amber",
  },
  {
    ...
    subtitle: "Program Platform with CI/CD and GitOps Delivery",
    category: "Product Infrastructure",
    description:
      "A TypeScript application and delivery system for an AI education platform, backed by deployment automation, drift checks, runtime image promotion, and production-style release controls.",
    stack: ["TypeScript", "CI/CD", "GitHub Actions", "Kubernetes", "GitOps", "Runtime Images"],
    pillars: ["Delivery", "Automation", "Product Ops"],
    outcomes: ["Path-filtered CI", "Deployable image promotion", "Runtime drift checks"],
    signal: "Active product platform",
  ...
  },
  {
    ...
    subtitle: "Secrets Operations and Platform Bootstrap Utilities",
    category: "Security Automation",
    description:
      "Python-based operational tooling around Vault-backed platform secrets, bootstrap workflows, environment promotion, and safe secret synchronization into Kubernetes workloads.",
    stack: ["Python", "Vault", "External Secrets", "Kubernetes", "Shell", "Runbooks"],
    pillars: ["Secrets", "Automation", "Bootstrap"],
    outcomes: ["ExternalSecret recovery", "Secret patch workflow", "Operational audit trail"],
    signal: "Security-critical platform tooling",
    repo: "vault-ops",
    visibility: "Private case study",
    accent: "amber",
  },
  {
    ...
    subtitle: "Security and Operations Assistant Prototype",
    category: "AIOps / SecOps",
    description:
      "A Python project exploring assistant-driven security and operations workflows, designed to turn operational context into safer triage, guided checks, and repeatable investigation paths.",
    stack: ["Python", "Automation", "Security Workflows", "Ops Intelligence"],
    pillars: ["SecOps", "AIOps", "Triage"],
    outcomes: ["Assistant workflow patterns", "Security-focused automation", "Reusable investigation flow"],
    signal: "Public prototype",
  ...
  },
  {
    ...
    description:
      "A microservices simulation using API gateway governance, event-driven integration, release automation, observability, and reliability testing to model production-grade service delivery.",
...
export const architectureScenarios = [
  {
    ...
    summary:
      "The portfolio platform is presented as a control-plane story: GitHub drives Argo CD, secrets flow through Vault and External Secrets, traffic enters through Cloudflare and Envoy Gateway, and Longhorn keeps stateful services recoverable.",
    metric: "5 GitOps apps healthy",
    nodes: [
      { id: "github", label: "GitHub", x: 7, y: 52, tone: "teal" },
      { id: "argocd", label: "Argo CD", x: 24, y: 28, tone: "teal" },
      { id: "vault", label: "Vault", x: 24, y: 75, tone: "amber" },
      { id: "envoy", label: "Envoy", x: 48, y: 18, tone: "teal" },
      { id: "workloads", label: "Workloads", x: 61, y: 52, tone: "teal" },
      { id: "longhorn", label: "Longhorn", x: 47, y: 83, tone: "amber" },
      { id: "observe", label: "Observe", x: 82, y: 31, tone: "teal" },
      { id: "cloudflare", label: "Cloudflare", x: 87, y: 66, tone: "amber" },
    ],
    edges: [
      ["github", "argocd", "sync"],
      ["argocd", "workloads", "apply"],
      ["vault", "workloads", "secrets"],
      ["cloudflare", "envoy", "edge"],
      ["envoy", "workloads", "route"],
      ["workloads", "longhorn", "state"],
      ["workloads", "observe", "signals"],
      ["observe", "github", "runbooks"],
    ],
  },
  {
    id: "aiops",
    label: "AIOps loop",
    title: "Human-approved AI operations workflow",
    summary:
      "A safer AIOps pattern: telemetry and logs are gathered first, AI produces an evidence-ranked recommendation, and execution remains approval-gated instead of blindly automated.",
    metric: "Read-only first",
    nodes: [
      { id: "signals", label: "Signals", x: 10, y: 45, tone: "teal" },
      { id: "logs", label: "Logs", x: 23, y: 72, tone: "teal" },
      { id: "memory", label: "Runbooks", x: 30, y: 24, tone: "amber" },
      { id: "reasoner", label: "Reasoner", x: 52, y: 48, tone: "teal" },
      { id: "review", label: "Review", x: 73, y: 27, tone: "amber" },
      { id: "action", label: "Action", x: 88, y: 56, tone: "teal" },
    ],
    edges: [
      ["signals", "reasoner", "metrics"],
      ["logs", "reasoner", "context"],
      ["memory", "reasoner", "policy"],
      ["reasoner", "review", "evidence"],
      ["review", "action", "approval"],
      ["action", "signals", "verify"],
    ],
  },
  {
    id: "delivery",
    label: "Delivery lane",
    title: "Image promotion and frontend/API cutover path",
    summary:
      "The production path connects CI, immutable image promotion, GitOps reconciliation, Netlify frontend deployment, and API routing into a visible release lane with rollback points.",
    metric: "Digest-pinned backend",
    nodes: [
      { id: "commit", label: "Commit", x: 8, y: 50, tone: "teal" },
      { id: "ci", label: "CI", x: 25, y: 25, tone: "teal" },
      { id: "registry", label: "Registry", x: 44, y: 50, tone: "amber" },
      { id: "gitops", label: "GitOps", x: 61, y: 24, tone: "teal" },
      { id: "api", label: "API", x: 78, y: 45, tone: "teal" },
      { id: "netlify", label: "Netlify", x: 61, y: 76, tone: "amber" },
      { id: "users", label: "Users", x: 91, y: 70, tone: "teal" },
    ],
    edges: [
      ["commit", "ci", "test"],
      ["ci", "registry", "build"],
      ["registry", "gitops", "digest"],
      ["gitops", "api", "rollout"],
      ["commit", "netlify", "deploy"],
      ["netlify", "users", "frontend"],
      ["api", "users", "api"],
    ],
  },
];
