              ...
              <div>
                ...
                <div className="mt-7 flex flex-wrap gap-2">
                  {asArray(project.pillars).map((item) => (
                    <span key={item} className="rounded-md border border-teal-300/15 bg-teal-300/[0.06] px-3 py-1.5 font-mono text-xs text-teal-200">{item}</span>
                  ))}
                </div>
              </div>
              <aside className="rounded-3xl border border-white/10 bg-white/[0.035] p-5">
                ...
                <div className="mt-4 space-y-3">
                  {artifactLinks.map(({ label, href, icon: Icon }) => (
                    <a key={label} href={href} target={href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="flex items-center justify-between gap-4 rounded-2xl border border-white/[0.07] bg-black/10 px-4 py-3 text-sm text-slate-300 hover:border-teal-300/30 hover:text-teal-200">
            ...
            <div className="mt-5 grid md:grid-cols-2 gap-4">
              {Object.entries(caseStudy.measurementNotes || {}).map(([key, value]) => (
                <div key={key} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4">
                  <div className="font-mono text-xs text-teal-300">{key}</div>
...
const CaseBlock = ({ title, body, highlight = false }) => (
  <article className={`rounded-3xl border p-5 ${highlight ? "border-teal-300/20 bg-teal-300/[0.06]" : "border-white/[0.07] bg-white/[0.025]"}`}>
