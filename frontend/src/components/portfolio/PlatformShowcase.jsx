      ...
      <div className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950/75 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.5)] backdrop-blur-xl">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative min-h-[300px] p-5 sm:p-7">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_15%,rgba(201,124,68,0.05),transparent_32%)]" />
            <div className="relative">
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs text-slate-200">
                  <ActiveIcon className="h-4 w-4 text-stone-300" />
                  {active.label}
                </span>
              </div>
              ...
              <div className="mt-6">
                {active.href ? (
                  active.href.startsWith("http") ? (
                    <a href={active.href} target="_blank" rel="noreferrer" className="group inline-flex min-h-11 items-center gap-2 rounded-full bg-stone-100 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-stone-50">
                      {active.action}
                      <ExternalLink className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
                    </a>
                  ) : (
                    <a href={active.href} className="group inline-flex min-h-11 items-center gap-2 rounded-full bg-stone-100 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-stone-50">
                      {active.action}
          ...
          <div className="border-t border-white/10 bg-black/20 p-3 lg:border-l lg:border-t-0">
            <div className="grid gap-2">
              {platforms.map((platform, index) => {
                ...
                return (
                  <button key={platform.id} type="button" onClick={() => setActiveId(platform.id)} className={`group min-h-24 rounded-2xl border p-4 text-left transition ${isActive ? "border-stone-400/30 bg-white/[0.08]" : "border-white/10 bg-white/[0.035] hover:bg-white/[0.07]"}`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex gap-3">
                        <span className={`grid h-9 w-9 place-items-center rounded-xl ${isActive ? "bg-stone-200 text-slate-950" : "bg-white/10 text-slate-300"}`}>
                      ...
                      <ArrowRight className={`mt-1 h-4 w-4 shrink-0 transition ${isActive ? "text-stone-300" : "text-slate-600 group-hover:text-slate-300"}`} />
                    </div>
                  </button>
                );
              })}
