      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="mt-10 mb-14 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <h2 className="text-3xl lg:text-4xl font-semibold tracking-[-0.02em] text-slate-50 max-w-3xl leading-tight">
            ...
            <span className="text-stone-300">AI-assisted platform engineering</span>.
          </h2>
        </div>

        <div className="relative">
          {/* Timeline rail */}
          <div className="absolute left-3 md:left-4 top-2 bottom-2 w-px bg-gradient-to-b from-stone-500/30 via-white/8 to-transparent" />

          <ol className="space-y-10">
            {experiences.map((exp, idx) => (
              <li key={exp.id} className="relative pl-12 md:pl-16">
                {/* Marker */}
                <span className="absolute left-0 md:left-1 top-1 h-7 w-7 rounded-full bg-[#0e1620] border border-stone-500/35 flex items-center justify-center">
                  <span className="h-2 w-2 rounded-full bg-stone-400" />
                </span>

                <div className="rounded-xl border border-white/5 bg-white/[0.02] hover:border-stone-400/18 transition-colors p-6 lg:p-7">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2 text-xs text-stone-400 font-mono uppercase tracking-wider">
                  ...
                  <ul className="mt-5 grid sm:grid-cols-2 gap-x-6 gap-y-2.5">
                    {exp.achievements.map((a) => (
                      ...
                      >
                        <CheckCircle2 className="h-4 w-4 mt-0.5 text-stone-400/80 flex-shrink-0" />
                        <span>{a}</span>
                      </li>
                    ))}
                  </ul>
