...
const badges = [
  ...
  { label: "ForgeWatch repo", href: "https://github.com/temitayocharles/sentinel-copilot", src: "https://img.shields.io/badge/ForgeWatch-public_repo-0f172a" },
  { label: "OpenLeaf repo", href: "https://github.com/temitayocharles/OpenLeaf-Reader-Platform", src: "https://img.shields.io/badge/OpenLeaf-public_repo-0f172a" },
];

const HiringManagerSummary = () => {
  return (
    <section id="hiring-summary" className="relative border-y border-white/5 bg-[#081018] py-20">
      <div className="max-w-7xl mx-auto px-6 lg:px-10 grid lg:grid-cols-[0.9fr_1.1fr] gap-10 items-start">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-teal-300/15 bg-teal-300/[0.06] px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-teal-200">
        ...
        <div className="space-y-4">
          {proofItems.map((item) => (
            <div key={item} className="flex gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4 text-sm leading-relaxed text-slate-300">
              <BadgeCheck className="mt-0.5 h-4 w-4 flex-none text-teal-300" />
              <span>{item}</span>
            </div>
          ))}
          <div className="rounded-2xl border border-white/[0.07] bg-black/10 p-4">
            ...
            <div className="flex flex-wrap gap-3">
              {badges.map((badge) => (
                <a key={badge.label} href={badge.href} target="_blank" rel="noreferrer" aria-label={`Open ${badge.label}`} className="inline-flex items-center gap-2 rounded-lg border border-white/[0.07] bg-white/[0.03] px-3 py-2 hover:border-teal-300/30">
